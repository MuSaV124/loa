...
async function verifyViewport(viewport) {
  ...
  assert.match(critDamageText, /스킬 · 표본 기술[\s\S]*원효과 \+180\.00% · 딜 지분/);
  assert.match(critDamageText, /스킬 · 상위 치피 기술[\s\S]*원효과 \+210\.00% · 딜 지분/);
  const critDamageTotal = Number((await critDamageGroup.locator('summary em').innerText()).replace(/[^\d.-]/g, ''));
  assert.ok(critDamageTotal > 200, `스킬별 치피의 딜 지분 가중값이 계산 요약에 포함되어야 합니다: ${critDamageTotal}`);
